...
# df_combined = pd.concat([df_2d_desc, df_fprint, df_maccs, df_mqn], axis='columns')
# df_combined.drop('SMILES', axis='columns', inplace=True)

# df_combined_scaled = pd.DataFrame(MinMaxScaler().fit_transform(df_combined), columns=df_combined.columns)

df_wo_fprint = pd.concat([df_2d_desc, df_maccs, df_mqn], axis='columns')
df_wo_fprint.drop('SMILES', axis='columns', inplace=True)
df_combined_scaled = pd.DataFrame(MinMaxScaler().fit_transform(df_wo_fprint), columns=df_wo_fprint.columns)


# df_combined_target = y1.astype(str) + "_" + y2.astype(str)

#######################################################################################################

X_train_PKM2, X_test_PKM2, y_train_PKM2, y_test_PKM2 = train_test_split(df_combined_scaled, y_PKM2,
                                                            test_size=0.2, stratify=y_PKM2)

X_train_ERK2, X_test_ERK2, y_train_ERK2, y_test_ERK2 = train_test_split(df_combined_scaled, y_ERK2, 
...
param_grid = {'n_estimators': [50, 100, 250],
              'max_features': [100, 150, 'sqrt', 'log2'],
              'max_depth': [2, 5, 10, 20],
              'min_samples_split': [2],
              'min_samples_leaf': [1]}
