from utils import descriptors_from_csv
import numpy as np
import pandas as pd
...
def train_model(x, y, model_type='RF', param_grid=None):
    if model_type == 'RF':
        ...
        if param_grid:
            ...
            parameters = {'n_estimators': list(range(1, 5)),
    ...
    m = GridSearchCV(clf, param_grid=parameters, scoring=scorers)
    m.fit(x, y)
    #print(m.best_score_)
...
def k_fold_training(x, y, model_type, random_state=7):
...
if __name__ == '__main__':
    ...
    use_ind_test = True  # Whether you want to use independent test set
    use_k_fold = False  # Whether you want to use k-fold cross-validation
    m_type = "RF"  # model type: "RF" or "LR"

    r_state = 7  # Random state for an attempt at reproducibility of results
    test_size = 0.3  # Size of test set within Cross-Validation (validation set)
    ind_test_size = 0.2  # Size of independent test set (if use_ind_test)
    ...
    if ind_test_size and use_k_fold:
    ...
    for target_name in target_names:
        ...
        if merge:
            ...
            if use_k_fold:
                # train
                model, acc_l, auc_l = k_fold_training(
